...
что я и дальше буду этим пользоваться. Пожалуй, всегда.
...
Стоит отметить, что если вам нужен обход блокировок только на одном компьютере,
...
уже стоит знать наизусть — `8.8.8.8, 8.8.4.4`. Раньше это действительно, но сегодня
провайдеры уже не дадут вам так просто себя обмануть.
...
HTTP-запрос к Google или Cloudflare.

Именно DNS-over-HTTPS мы и собираемся настроить в первую очередь.
...
раз из-за этого чуть не поругался с тех.поддержкой, а оказалось, что это я сам дурак.
...
другого способа нет. В качестве протокола предлагаю использовать WireGuard —
легковесный VPN-протокол, который довольно легко настраивать.
...
господа спамеры), и некоторые ресурсы, типа Пикабу, сразу же заранее отвечают
ошибкой 403. Намного проще купить готовый VPN, и пусть лучше кто-то другой за меня
...
[поддержка WireGuard была добавлена основной состав ядра Linux](
...
Открываем его при помощи `vi` и дописываем в конец две секции вот такого содержания:
...
6. После этого трафик к заблокированным сайтам должен побежать через VPN.
    Честно говоря, мне ещё почему-то после этого пришлось перезапустить роутер.
