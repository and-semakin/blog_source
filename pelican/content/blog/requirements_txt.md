...
Команда `pip freeze` выводить все установленные в интерпретатор сторонние пакеты.
Заметьте, что в список попали не только прямые зависимости (`pyowm`), но
и подзависимости — в целом, это не проблема, но полезно иметь в виду.
...
* `pip freeze`:
    ...
    * возможно, слишком подробно, потому что в файл попадет
        всё дерево зависимостей, а не только прямые зависимости.

В большинстве ситуаций оптимальный подход находится где-то посередине:
можно сгенерировать начальную версию файла при помощи `pip freeze` и
допилить её затем руками.
...
Конечно, я лишь затронул верхушку айсберга. На самом деле, `requirements`-файлы
могут намного больше и всё немножко сложнее:
